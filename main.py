...
with st.form("spamdetector",clear_on_submit=True):
    sms=st.text_area("SMS Giriniz")
    buton=st.form_submit_button("Kontrol Et")

    if buton:
        df=df.append({"v1":1,"v2":sms},ignore_index=True)
        df['v2'] = df['v2'].str.lower()
        noktalamalar = string.punctuation
        for nokta in noktalamalar:
            df['v2'] = df['v2'].str.replace(nokta, "")
        ...
        for stop in stopwords:
            stop = " " + stop + " "
            df['v2'] = df['v2'].str.replace(stop, " ")

        cvt = CountVectorizer(max_features=150)
        x = cvt.fit_transform(df['v2']).toarray()
